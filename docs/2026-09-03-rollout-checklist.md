...
## Standing rule for `feat/subhanu`

**After every merge to `main`, pull `main` back into this branch before doing
more work.**

The repo squash-merges. A squash replays the branch's commits as one *new*
commit on `main`, so the originals still sitting on the branch never match by
hash. A long-lived branch therefore diverges the instant one of its PRs lands,
and the next PR shows the already-merged commit again with an inflated diff.

Missed after PR #15 merged on 2 Sept: the branch went 2 behind / 3 ahead with
`8e1c426` still as the merge base, PR #16 listed `353c130` even though its
content was already on `main` as `18f35af`, and reconciling it produced seven
conflicts instead of the one it would have been.

Resolving that class of conflict: they are all "both sides added here", and the
branch side is the superset (`main`'s unique lines are the *older* versions the
branch replaced). So `git checkout --ours <files>` is right, with `VERSION` left
to auto-merge since Jenkins owns it.
